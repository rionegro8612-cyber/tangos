...
export async function createUserWithKyc(input: CreateUserInput): Promise<number> {
...
export async function findByPhone(phone: string) {
  const rows = await query<{ id: number }>(`SELECT id FROM users WHERE phone_e164_norm = $1`, [phone]);
