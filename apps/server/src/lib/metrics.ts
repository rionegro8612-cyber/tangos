...
let collectInterval: any = null;
export const register = new client.Registry();

const isTestEnv =
  process.env.NODE_ENV === 'test' || !!process.env.JEST_WORKER_ID; // ★ 이중 가드

const isDevEnv = 
  process.env.NODE_ENV === 'development' && process.env.DISABLE_METRICS === 'true';
...
export function initMetrics() {
  if (isTestEnv || isDevEnv) return { enabled: false };
  if (!collectInterval) {
    collectInterval = client.collectDefaultMetrics({
 ...
 * HTTP 요청 완료 시 호출
 */
export function endHttpRequestTimer(
  timer: ReturnType<typeof startHttpRequestTimer>,
  statusCode: number,
) {
  ...
  // 총 요청 수 카운터
  httpRequestTotal.labels(timer.method, timer.route, String(status), timer.endpoint).inc();

  // 에러율 계산 (5xx 응답)
  if (status >= 500) {
    // 🆕 .get() 메서드 제거하고 단순히 에러율을 100%로 설정
    // Counter 메트릭은 .get() 메서드를 지원하지 않음
    httpErrorRate.labels(timer.method, timer.route, timer.endpoint).set(100); // 5xx 응답이 발생하면 에러율을 100%로 설정
  }
}
...
export function recordOtpSend(status: "success" | "fail", provider: string, carrier: string) {
  otpSendTotal.labels(status, provider, carrier).inc();
}
...
export function recordOtpVerify(status: "success" | "fail", reason?: string) {
  otpVerifyTotal.labels(status, reason || "unknown").inc();

  if (status === "fail" && reason) {
    otpFailureReasons.labels(reason, "OTP_VERIFY_FAIL").inc();
  }
}
...
export function recordRateLimitExceeded(scope: "phone" | "ip" | "combo", type: string) {
  rateLimitExceeded.labels(scope, type).inc();
}
...
export function recordUserRegistration(status: "success" | "fail", reason?: string) {
  userRegistrationTotal.labels(status, reason || "unknown").inc();

  if (status === "success") {
    dailyNewUsers.labels(new Date().toISOString().split("T")[0]).inc();
  }
}
...
export function recordUserLogin(status: "success" | "fail", reason?: string) {
  userLoginTotal.labels(status, reason || "unknown").inc();
}
...
export function recordRedisOperation(operation: string, keyPattern: string, duration: number) {
  redisOperationDuration.labels(operation, keyPattern).observe(duration);
}
