...
async function verifyWithNice(request: KycRequest): Promise<KycResponse> {
  ...
  try {
    // NICE API 호출 (실제 구현 시)
    const response = await kycClient.post<any>(
      `${NICE_API_URL}/digital/niceid/api/v1.0/common/crypto/token`,
      {
        dataHeader: {
          CNTY_CD: "ko",
        },
...
export async function verifyKyc(request: KycRequest): Promise<KycResponse> {
  ...
  try {
    ...
    if (
      passError instanceof StandardError &&
 ...
 * KYC 결과 검증
 */
export function validateKycResult(
  result: KycResponse,
  expectedName: string,
