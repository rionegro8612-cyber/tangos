...
export class PiiManager {
  ...
  private async executeDataDeletion(
    ...
    try {
      ...
      if (success) {
        console.log(
          `[PII] Successfully deleted data from ${location.table} for ${location.identifierField}: ${this.maskIdentifier(location.identifierValue)}`,
        );
      } else {
        console.log(
          `[PII] Failed to delete data from ${location.table} for ${location.identifierField}: ${this.maskIdentifier(location.identifierValue)}`,
        );
      }
  ...
  getDataLocations(
    dataType?: string,
  ): Map<string, PiiDataLocation[]> | PiiDataLocation[] | undefined {
    if (dataType) {
    ...
    return {
      deletionRequests: {
        ...
        pending: Array.from(this.deletionRequests.values()).filter((r) => r.status === "pending")
          .length,
        processing: Array.from(this.deletionRequests.values()).filter(
          (r) => r.status === "processing",
        ).length,
        completed: Array.from(this.deletionRequests.values()).filter(
      ...
      correctionRequests: {
        ...
        pending: Array.from(this.correctionRequests.values()).filter((r) => r.status === "pending")
          .length,
        approved: Array.from(this.correctionRequests.values()).filter(
          (r) => r.status === "approved",
        ).length,
        rejected: Array.from(this.correctionRequests.values()).filter(
          (r) => r.status === "rejected",
        ).length,
      },
