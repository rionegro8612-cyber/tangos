...
import { getSystemHealth, isAlive, isReady, shouldRollback } from '../lib/health';
 ...
 */
healthRouter.get('/_ping', (_req, res) => {
  res.json({ ok: true });
});
 ...
 * 자동 배포 시스템에서 사용
 */
healthRouter.get('/health/rollback', async (_req, res) => {
  try {
    const rollbackInfo = await shouldRollback();
