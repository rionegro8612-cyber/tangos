...
const localPath = path_1.default.join(process.cwd(), "resources", "regions_kr.sample.json");
function normalizeQ(q) {
    return q.trim().replace(/\s+/g, " ");
}
function norm(label, lat, lng, code, source = "local") {
    const L = (label || "").trim();
    if (!L)
        return null;
...
async function searchLocation(qRaw) {
