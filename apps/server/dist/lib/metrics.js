...
const isTestEnv = process.env.NODE_ENV === 'test' || !!process.env.JEST_WORKER_ID; // ★ 이중 가드
const isDevEnv = process.env.NODE_ENV === 'development' && process.env.DISABLE_METRICS === 'true';
// 디버깅을 위한 로그
console.log('[METRICS DEBUG] Environment check:', {
...
function initMetrics() {
    ...
    if (!collectInterval) {
        collectInterval = prom_client_1.default.collectDefaultMetrics({
...
// 테스트나 개발 환경이 아닌 경우에만 메트릭 수집 시작
// 개발 환경에서는 강제로 비활성화
if (!isTestEnv && !isDevEnv && process.env.NODE_ENV !== 'development') {
    initMetrics();
}
else {
    console.log('[METRICS] Metrics collection disabled for current environment');
}
// ===== HTTP 요청 메트릭 =====
/**
 * HTTP 요청 지연시간 히스토그램
...
function endHttpRequestTimer(timer, statusCode) {
    ...
    exports.httpRequestDuration
        ...
        .observe(duration);
    // 총 요청 수 카운터
    exports.httpRequestTotal.labels(timer.method, timer.route, String(status), timer.endpoint).inc();
    // 에러율 계산 (5xx 응답)
    if (status >= 500) {
        // 🆕 .get() 메서드 제거하고 단순히 에러율을 100%로 설정
        // Counter 메트릭은 .get() 메서드를 지원하지 않음
        exports.httpErrorRate.labels(timer.method, timer.route, timer.endpoint).set(100); // 5xx 응답이 발생하면 에러율을 100%로 설정
    }
}
/**
 * OTP 전송 메트릭 업데이트
...
function recordOtpSend(status, provider, carrier) {
    exports.otpSendTotal.labels(status, provider, carrier).inc();
}
/**
 * OTP 검증 메트릭 업데이트
 */
function recordOtpVerify(status, reason) {
    exports.otpVerifyTotal.labels(status, reason || "unknown").inc();
    if (status === "fail" && reason) {
        exports.otpFailureReasons.labels(reason, "OTP_VERIFY_FAIL").inc();
    }
}
/**
 * 레이트리밋 메트릭 업데이트
...
function recordRateLimitExceeded(scope, type) {
    exports.rateLimitExceeded.labels(scope, type).inc();
}
/**
 * 사용자 등록 메트릭 업데이트
 */
function recordUserRegistration(status, reason) {
    exports.userRegistrationTotal.labels(status, reason || "unknown").inc();
    if (status === "success") {
        exports.dailyNewUsers.labels(new Date().toISOString().split("T")[0]).inc();
    }
}
/**
 * 사용자 로그인 메트릭 업데이트
...
function recordUserLogin(status, reason) {
    exports.userLoginTotal.labels(status, reason || "unknown").inc();
}
/**
 * 데이터베이스 쿼리 메트릭 업데이트
 */
function recordDbQuery(operation, table, duration) {
    exports.dbQueryDuration.labels(operation, table).observe(duration);
}
/**
 * Redis 작업 메트릭 업데이트
 */
function recordRedisOperation(operation, keyPattern, duration) {
    exports.redisOperationDuration.labels(operation, keyPattern).observe(duration);
}
// ===== 메트릭 수집기 설정 =====
/**
 * 커스텀 메트릭 수집기
...
if (!isTestEnv && !isDevEnv) {
    ...
    console.log("[METRICS] Metrics disabled for", isTestEnv ? "test" : "development", "environment");
}
