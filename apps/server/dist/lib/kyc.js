...
exports.validateKycResult = validateKycResult;
...
function calculateAge(birthDate) {
    const birth = new Date(parseInt(birthDate.substring(0, 4)), parseInt(birthDate.substring(4, 6)) - 1, parseInt(birthDate.substring(6, 8)));
...
async function verifyWithPass(request) {
    ...
    try {
        // PASS API 호출 (실제 구현 시)
        const response = await httpClient_1.kycClient.post(`${PASS_API_URL}/kyc/verify`, {
 ...
 */
async function verifyKyc(request) {
    ...
    try {
        ...
        try {
            ...
            if (niceError instanceof errorCodes_1.StandardError) {
                throw niceError;
            }
 ...
 * KYC 결과 검증
 */
function validateKycResult(result, expectedName, expectedBirth) {
