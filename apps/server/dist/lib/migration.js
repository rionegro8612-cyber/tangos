...
exports.executeCleanupMigration = executeCleanupMigration;
...
async function ensureMigrationTable() {
  ...
  `;
    await db_1.pool.query(createTableSql);
}
/**
 * 마이그레이션 단계 실행
 */
async function executeMigrationStep(step) {
    ...
    try {
        ...
        // SQL 실행
        await db_1.pool.query(step.sql);
 ...
 * 마이그레이션 롤백
 */
async function rollbackMigrationStep(step) {
    if (!step.rollbackSql) {
        return {
            success: false,
            stepId: step.id,
    ...
    try {
        ...
        // 롤백 SQL 실행
        await db_1.pool.query(step.rollbackSql);
        const executionTime = Date.now() - startTime;
        // 롤백 기록
        await db_1.pool.query(`UPDATE migration_history 
       SET rollback_executed = true, rollback_at = NOW() 
       WHERE step_id = $1 AND success = false`, [step.id]);
        return {
            success: true,
            stepId: step.id,
 ...
 * 2단계: 안정화 후 실행
 */
async function executeCleanupMigration(plan) {
    ...
    if (lastMigration.lastExecuted) {
        ...
        if (hoursSinceLastMigration < 24) {
            throw new Error(`Cleanup migration requires 24 hours stabilization period. Last migration: ${hoursSinceLastMigration.toFixed(1)} hours ago`);
        }
    }
    // 에러율 확인 (5% 미만이어야 함)
