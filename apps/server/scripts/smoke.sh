...
echo "pgcrypto 확장 및 핵심 테이블 확인..."

# PostgreSQL 연결 테스트 (Windows 환경 대응)
echo "  PostgreSQL 연결 테스트 중..."
if docker exec tango-postgres psql -U tango -d tango -c "SELECT extname FROM pg_extension WHERE extname = 'pgcrypto';" 2>/dev/null | grep -q "pgcrypto"; then
    echo -e "  ${GREEN}✅ pgcrypto 확장 설치됨${NC}"
    ((PASSED++))
else
    echo -e "  ${RED}❌ pgcrypto 확장 없음${NC}"
    ((FAILED++))
fi

# 핵심 테이블 확인
echo "  핵심 테이블 확인 중..."
if docker exec tango-postgres psql -U tango -d tango -c "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('posts', 'comments', 'users');" 2>/dev/null | grep -q "posts\|comments\|users"; then
    echo -e "  ${GREEN}✅ 핵심 테이블 존재 확인${NC}"
    ((PASSED++))
else
    echo -e "  ${RED}❌ 핵심 테이블 없음${NC}"
    ((FAILED++))
fi
